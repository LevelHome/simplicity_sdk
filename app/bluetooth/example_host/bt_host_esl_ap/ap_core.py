...
class AccessPoint:
    ...
    def ap_network(self, export_file=None, import_file=None, exclusive_mode=None):
        ...
        if all(value is None for value in [export_file, import_file, exclusive_mode]):
            self.log.info(
                ...
                if self.exclusive_list is None
        ...
        try:
            ...
            if exclusive_mode is not None:
                ...
                if not exclusive_mode:
                    ...
                    self.log.warning(
                        "Exclusive network configuration is enabled with no known ESLs. Please consider importing the network config or configuring the ESLs manually!"
                    )
                else:
                    self.log.info(
                ...
                self.exclusive_list = None if exclusive_mode is False else True
        ...
        if self.esl_queued_commands:
            ...
            if self.esl_command_queue_lock.acquire(False):
                ...
                for gid in subevents:
                    if (
                        gid not in self.esl_queued_commands
                    ):  # go to next if there's nothing to send this current group
                        continue
                    commands = []
                    pop_list = []
                    ...
                    if data is not None:
                        self.lib.pawr_set_data(
                            self.pawr_handle, gid, len(commands), data
                        )
    ...
    def synchronization_handler(self):
        ...
        syncronized_tags = self.tag_db.list_esl_state(EslState.SYNCHRONIZED)
        response_timeout = (1.5 * self.pa_timer_interval) + (
            0.00125 * self.response_slot_delay
        )
        ...
        for tag in syncronized_tags:
            ...
            if tag.group_id in self.esl_pending_commands:
                ...
                for cmd in current_pending_commands:
                    if (
                        cmd.esl_id == tag.esl_id
                        and cmd.group_id == tag.group_id
    ...
    def dequeue(self):
        ...
        while True:
            ...
            try:
                ...
                event_name = esl_lib.get_enum(enum_prefix, event.evt_code)[
                    len(enum_prefix) :
                ].lower()
    ...
    def esl_event_system_boot(self, evt: esl_lib.EventSystemBoot):
        ...
        for tag in self.tag_db.all():
            if (
                ...
                self.key_db.delete_ltk(tag.ble_address)
            tag.reset()
        if isinstance(self.exclusive_list, str):
            json = ap_json_helper.JSONHelper()
            list = json.import_network_config(
                self.lib, self.key_db, self.tag_db, self.exclusive_list
            )

            if (group_count := len(list)) == 0:
                self.log.warning(
                    "Exclusive mode is disabled because no devices could be loaded from the exclusive list file: %s.",
                    self.exclusive_list,
                )
                self.exclusive_list = None
            else:
                device_count = self.tag_db.device_count
                self.log.warning(
                    "Based on the data loaded from file %s, exclusive mode is enabled for %d device%s in %d group%s.",
                    self.exclusive_list,
                    device_count,
                    "" if device_count == 1 else "s",
                    group_count,
    ...
    def esl_event_tag_found(self, evt: esl_lib.EventTagFound):
        ...
        if evt.rssi > self.rssi_threshold:
            ...
            if tag is None and len(self.tag_db.list_advertising()) < MAX_ADVERTISER_TIMEOUT_THREADS:
                tag = self.tag_db.add(self.lib, evt.address)
                if self.exclusive_list is not None:
    ...
    def esl_event_image_transfer_finished(
        self, evt: esl_lib.EventImageTransferFinished
    ):
        """Generic handler for the image transfer finished event of the ESL library"""
    ...
    def esl_event_pawr_response(self, evt: esl_lib.EventPawrResponse):
        ...
        if tag is not None:
            ...
            self.log.warning(
                "PAwR response received but unable to decrypt: 0x" + evt.data.hex()
            )
        ...
        for key in self.esl_pending_commands:
            ...
            for item in self.esl_pending_commands[key]:
                if (now - item.timestamp) > limit:
                    cmd = item
                    self.log.warning(
                        "Unresponded command (0x%s) in subevent %s slot %d removed from queue.",
                        cmd.params.hex(),
                        key,
                        cmd.slot_number,
                    )
            ...
            for cmd in self.esl_pending_commands[subevent]:
                ...
                if cmd.slot_number == response_slot and cmd.group_id == subevent:
                    ...
                    if tag is not None:
                        ...
                        self.log.warning(
                            "Unable to identify sender tag ID %d in group %d",
                            cmd.esl_id,
                            cmd.group_id,
                        )
                    return tag
            ...
            if subevent in self.esl_pending_commands:
                for cmd in self.esl_pending_commands[subevent]:
                    ...
                    if cmd.slot_number == response_slot and cmd.group_id == subevent:
                        ...
                        if (
                            tag is not None
                            and self.esl_pending_commands[subevent].index(cmd)
                            not in pop_list
                        ):
                            ...
                            if cmd.opcode_valid(response_data[0]):
                                ...
                                if (
                                    response_data[0] == TLV_RESPONSE_BASIC_STATE
                                    and tag.pending_unassociate
                                ):
                                    tag.block(
                                        elw.ESL_LIB_STATUS_UNASSOCIATED
