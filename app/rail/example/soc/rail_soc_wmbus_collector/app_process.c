...
static void print_rx_packets(RAIL_Handle_t rail_handle)
{
  ...
  while (rx_packet_handle != RAIL_RX_PACKET_HANDLE_INVALID) {
    ...
    const sl_rail_sdk_wmbus_dll_header_t *dll_header = (sl_rail_sdk_wmbus_dll_header_t*)rx_fifo;
    const sl_rail_sdk_wmbus_stl_header_t *stl_header = (sl_rail_sdk_wmbus_stl_header_t*)(rx_fifo + sizeof(sl_rail_sdk_wmbus_dll_header_t));
    ...
    if (stl_header->ci_field == WMBUS_CI_EN13757_3_APPLICATION_SHORT) {
      uint8_t *payload_start = rx_fifo + sizeof(sl_rail_sdk_wmbus_dll_header_t) + sizeof(sl_rail_sdk_wmbus_stl_header_t);
      ...
      print_blocks(rx_fifo + sizeof(sl_rail_sdk_wmbus_dll_header_t), dll_header->lField - sizeof(sl_rail_sdk_wmbus_dll_header_t) + 1);
    }
