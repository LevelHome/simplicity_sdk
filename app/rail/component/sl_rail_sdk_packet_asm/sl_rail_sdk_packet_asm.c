/***************************************************************************//**
 * @file
 * @brief sl_rail_sdk_packet_asm.c
 *******************************************************************************
 * # License
 ...
 *****************************************************************************/
static void sl_rail_sdk_ieee802154_set_length(uint8_t *payload, uint16_t length);
 ...
 *****************************************************************************/
static uint16_t sl_rail_sdk_ieee802154_get_length(uint8_t *payload);
 ...
 *****************************************************************************/
static inline sl_rail_sdk_ble_packet_size_t
calc_ble_header_length(const sl_rail_sdk_ble_packet_size_t payload_len);
 ...
 *****************************************************************************/
static inline sl_rail_sdk_ble_packet_size_t
calc_ble_payload_length(const sl_rail_sdk_ble_packet_size_t header_len);
                                                        ...
                                                        uint8_t *frame_buffer)
{
  ...
  if ((phr_cfg & SL_RAIL_SDK_IEEE802154G_PHR_CRC_2_BYTE)
      == SL_RAIL_SDK_IEEE802154G_PHR_CRC_2_BYTE) {
    crc_size = SL_RAIL_SDK_IEEE802154G_CRC_LENGTH_2BYTE;
  } else {
    crc_size = SL_RAIL_SDK_IEEE802154G_CRC_LENGTH_4BYTE;
  }

  // check the size of the payload
  if (payload_size
      > (SL_RAIL_SDK_IEEE802154G_LEN_MAX - crc_size - SL_RAIL_SDK_IEEE802154_MHR_LENGTH)) {
  ...
  length = SL_RAIL_SDK_IEEE802154_MHR_LENGTH + payload_size + crc_size;
  sl_rail_sdk_ieee802154_set_length(tmp_data, length);

  // set the rest of the PHR field
  tmp_data[0] |= phr_cfg;

  // step forward to MHR
  tmp_data += SL_RAIL_SDK_IEEE802154G_PHR_LENGTH;
                                                  ...
                                                  uint8_t *frame_buffer)
{
  ...
  // PHR
  crc_size = SL_RAIL_SDK_IEEE802154_CRC_LENGTH;

  // check the size of the payload
  if (payload_size
      > (SL_RAIL_SDK_IEEE802154_LEN_MAX - crc_size - SL_RAIL_SDK_IEEE802154_MHR_LENGTH)) {
    // the payload is too large
#if defined(SL_CATALOG_APP_LOG_PRESENT)
  ...
  // step forward to MHR
  tmp_data += SL_RAIL_SDK_IEEE802154_PHR_LENGTH;
                                                           ...
                                                           uint8_t *frame_buffer)
{
  ...
  // get the PHR config
  *phr_cfg = tmp[0] & SL_RAIL_SDK_IEEE802154G_PHR_GET_PHR_CFG_MASK;
  ...
  // get length
  length = sl_rail_sdk_ieee802154_get_length(tmp);

  // step forward
  tmp += SL_RAIL_SDK_IEEE802154G_PHR_LENGTH;
  ...
  *payload_size = length - crc_size - SL_RAIL_SDK_IEEE802154_MHR_LENGTH;
                                                     ...
                                                     uint8_t *frame_buffer)
{
  ...
  // step forward
  tmp += SL_RAIL_SDK_IEEE802154_PHR_LENGTH;
...
calc_ble_header_length(const sl_rail_sdk_ble_packet_size_t payload_len)
{
  return (payload_len + SL_RAIL_SDK_BLE_HEADER_LEN_BASE);
}
...
calc_ble_payload_length(const sl_rail_sdk_ble_packet_size_t header_len)
{
  if (header_len < SL_RAIL_SDK_BLE_HEADER_LEN_BASE) {
    return 0;
  }
  return (header_len - SL_RAIL_SDK_BLE_HEADER_LEN_BASE);
}

/*******************************************************************************
 * Prepare BLE advertising packet
 ******************************************************************************/
void sl_rail_sdk_ble_prepare_packet(sl_rail_sdk_ble_advertising_packet_t *packet, const uint8_t *payload, const sl_rail_sdk_ble_packet_size_t payload_length)
{
  sl_rail_sdk_ble_packet_size_t padding_size;

  if (packet == NULL || payload == NULL || payload_length == 0 || payload_length > SL_RAIL_SDK_BLE_PAYLOAD_LEN_MAX) {
#if defined(SL_CATALOG_APP_LOG_PRESENT)
    app_log_warning("sl_rail_sdk_ble_prepare_packet ERR: parameter\r\n");
#endif
    return;
  }

  // BLE advertisement header
  packet->header.type = SL_RAIL_SDK_BLE_HEADER_LSB;   //BLE_BLE_ADV_NONCONN_IND
  ...
  packet->flags.advertising_type = SL_RAIL_SDK_BLE_ADSTRUCT_TYPE_FLAG;   // AD type: Flags
  packet->flags.flags = SL_RAIL_SDK_BLE_DISABLE_BR_EDR | SL_RAIL_SDK_BLE_GENERAL_DISCOVERABLE_MODE;   // Flags: BR/EDR is disabled, LE General Discoverable Mode
  ...
  packet->manufactSpec.length = sizeof(packet->manufactSpec.advertising_type)
                                ...
                                + payload_length;
  packet->manufactSpec.advertising_type = SL_RAIL_SDK_BLE_ADSTRUCT_TYPE_MANUFACTURER_SPECIFIC;   // AD type: Manufacturer Specific Data
  packet->manufactSpec.company_id = SL_RAIL_SDK_BLE_COMPANY_ID;
  ...
  if (payload_length < SL_RAIL_SDK_BLE_PAYLOAD_LEN_MIN) {
    padding_size = SL_RAIL_SDK_BLE_PAYLOAD_LEN_MIN - payload_length;
 ...
 * Get BLE packet size
 ******************************************************************************/
sl_rail_sdk_ble_packet_size_t sl_rail_sdk_ble_get_packet_size(const sl_rail_sdk_ble_advertising_packet_t *packet)
{
  // Check arguments
  if (packet == NULL) {
#if defined(SL_CATALOG_APP_LOG_PRESENT)
    app_log_warning("sl_rail_sdk_ble_get_packet_size ERR: parameter\r\n");
#endif
    return 0;
  }
  return (sizeof(sl_rail_sdk_ble_advertising_packet_t) - SL_RAIL_SDK_BLE_PAYLOAD_LEN_MAX + sl_rail_sdk_ble_get_payload_len(packet));
}

/*******************************************************************************
 * Get BLE Packet from buffer
 ******************************************************************************/
void sl_rail_sdk_ble_copy_packet_from_buff(sl_rail_sdk_ble_advertising_packet_t *packet, const uint8_t *rx_data)
{
  // Check arguments
  if (packet == NULL || rx_data == NULL) {
#if defined(SL_CATALOG_APP_LOG_PRESENT)
    app_log_warning("sl_rail_sdk_ble_get_packet ERR: parameter\r\n");
#endif
    packet = NULL;
    return;
  }
  memcpy(packet, rx_data, sizeof(sl_rail_sdk_ble_advertising_packet_t));
}

/*******************************************************************************
 * Get BLE Packet pointer from data buffer
 ******************************************************************************/
sl_rail_sdk_ble_advertising_packet_t * sl_rail_sdk_ble_get_packet(uint8_t *data)
{
  return (sl_rail_sdk_ble_advertising_packet_t *) data;
}

/*******************************************************************************
 * Copy BLE packet payload
 ******************************************************************************/
void sl_rail_sdk_ble_copy_payload(sl_rail_sdk_ble_advertising_packet_t *packet, uint8_t *dest, const sl_rail_sdk_ble_packet_size_t payload_length)
{
  // Check arguments
  if (packet == NULL || dest == NULL || payload_length == 0) {
#if defined(SL_CATALOG_APP_LOG_PRESENT)
    app_log_warning("sl_rail_sdk_ble_copy_payload ERR: parameter\r\n");
#endif
    return;
  }
  memcpy(dest, packet->manufactSpec.payload, (payload_length > SL_RAIL_SDK_BLE_PAYLOAD_LEN_MAX) ? SL_RAIL_SDK_BLE_PAYLOAD_LEN_MAX : payload_length);
}

/*******************************************************************************
 * Get BLE payload
 ******************************************************************************/
uint8_t *sl_rail_sdk_ble_get_payload(sl_rail_sdk_ble_advertising_packet_t *packet)
{
  if (packet == NULL) {
    return NULL;
  }
 ...
 * Get BLE payload len
 ******************************************************************************/
inline sl_rail_sdk_ble_packet_size_t sl_rail_sdk_ble_get_payload_len(const sl_rail_sdk_ble_advertising_packet_t *packet)
{
  if (packet == NULL) {
    return 0;
  }
