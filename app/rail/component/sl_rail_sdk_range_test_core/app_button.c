...
#define BUTTON_LONG_PRESS_WAIT_MS ((uint16_t) 500)
...
/// in every @BUTTON_LONG_PRESS_WAIT_MS millisecond
 ...
 *
 * @param[in] button_number The selected button's number
...
static void button_long_press_callback(sl_sleeptimer_timer_handle_t *handle, void *data)
{
  ...
  if (handle == &button_0_long_press_timer) {
    button_0_wait_for_release = false;
    button_0_delay = sl_sleeptimer_ms_to_tick(BUTTON_LONG_PRESS_WAIT_MS);
    sl_button_on_change(&sl_button_btn0);
  } else if (handle == &button_1_long_press_timer) {
    button_1_wait_for_release = false;
    button_1_delay = sl_sleeptimer_ms_to_tick(BUTTON_LONG_PRESS_WAIT_MS);
