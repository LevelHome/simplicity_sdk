...
volatile bool service_received = false;
...
/// Flag indicating if the transmission of the packets has been finished
/// (when all the packets has been sent)
static bool tx_elapsed = false;
// -----------------------------------------------------------------------------
 ...
 *
 * @param[in] next_state What should be the next state in the state machine
 ******************************************************************************/
  ...
  switch (state) {
    ...
    case START_MEASUREMENT:
      ...
      if (range_test_settings.radio_mode == RADIO_MODE_RX) {
        ...
        set_next_state(RECEIVE_MEASUREMENT);
        receive_setup_radio();
        ...
        if (!range_test_measurement.tx_is_running) {
          if (range_test_settings.remote_config_enable) {
            receive_service_packet();
          }
        }
        set_next_state(SEND_MEASUREMENT);
#if defined(SL_CATALOG_RANGE_TEST_DMP_COMPONENT_PRESENT)
    ...
    case RECEIVE_MEASUREMENT:
      refresh_screen = receive_measurement();
      if (refresh_screen) {
#if defined(SL_CATALOG_GLIB_PRESENT)
        graphics_draw_rx_screen();
#endif
      ...
      if (get_and_clear_button_state(BUTTON_1)) {
        ...
        set_next_state(MENU_SCREEN);
        refresh_screen = true;
        get_and_clear_button_state(BUTTON_0);
      }
#endif
      break;
    case SEND_MEASUREMENT:
      if (range_test_measurement.tx_is_running) {
        refresh_screen = send_measurement();
        if (!range_test_measurement.tx_is_running) {
          tx_elapsed = true;
        }
      }
      ...
      if (get_and_clear_button_state(BUTTON_1)) {
...
#endif
      } else if (get_and_clear_button_state(BUTTON_0) || service_received) {
        if (service_received) {
        ...
        app_task_notify();
#endif
        if (range_test_measurement.tx_is_running == false) {
          if (range_test_settings.remote_config_enable) {
            receive_service_packet();
          }
          range_test_reset_values();
