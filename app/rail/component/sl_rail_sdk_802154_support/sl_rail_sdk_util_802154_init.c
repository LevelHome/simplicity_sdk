...
static RAIL_Handle_t sl_rail_sdk_handle = RAIL_EFR32_HANDLE;
/// RAIL schedule configuration
static RAILSched_Config_t sl_rail_sdk_sched_config;
/// RAIL configuration
static RAIL_Config_t sl_rail_sdk_memory_config = {
  .eventsCallback = &sli_rail_util_on_event,
  .scheduler = &sl_rail_sdk_sched_config,
};
...
void sl_rail_sdk_util_init(void)
{
  ...
  sl_rail_sdk_handle = RAIL_Init(&sl_rail_sdk_memory_config,
  ...
  status = RAIL_ConfigData(sl_rail_sdk_handle, &data_config);
  app_assert((RAIL_STATUS_NO_ERROR == status),
             "RAIL_ConfigData failed, return value: %lu",
             status);

  // configures the channels
  const RAIL_ChannelConfig_t *channel_config = NULL;
  (void) RAIL_ConfigChannels(sl_rail_sdk_handle,
                             channel_config,
  ...
  status = RAIL_ConfigCal(sl_rail_sdk_handle,
                          0U
                          | (0
                             ? RAIL_CAL_TEMP : 0U)
                          | (0
                             ? RAIL_CAL_ONETIME : 0U));
  app_assert((RAIL_STATUS_NO_ERROR == status),
             "RAIL_ConfigCal failed, return value: %lu",
             status);

  // configures the using RAIL events
  status = RAIL_ConfigEvents(sl_rail_sdk_handle,
                             RAIL_EVENTS_ALL,
  ...
  app_assert((RAIL_STATUS_NO_ERROR == status),
             "RAIL_ConfigEvents failed, return value: %lu",
             status);
}
