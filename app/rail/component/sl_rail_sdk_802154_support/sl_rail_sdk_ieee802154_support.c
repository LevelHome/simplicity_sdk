/***************************************************************************//**
 * @file sl_rail_sdk_ieee802154_support.c
 * @brief RAIL SDK - RAIL IEEE 802.15.4 support Component
 *******************************************************************************
 * # License
 ...
 *****************************************************************************/
bool sl_rail_sdk_ieee802154_get_ack_status(void);
...
static sl_rail_sdk_ieee802154_std_t standard_protocol =
  SL_RAIL_SDK_IEEE802154_STD_IEEE802154_2P4GHZ;
                                             ...
                                             sl_rail_sdk_util_802154_protocol_type_t protocol)
{
  ...
  int16_t status = SL_RAIL_SDK_IEEE802154_OK;
  ...
  switch (protocol) {
    ...
    case SL_RAIL_SDK_UTIL_PROTOCOL_IEEE802154_2P4GHZ_ANTDIV_COEX:
      standard_protocol = SL_RAIL_SDK_IEEE802154_STD_IEEE802154_2P4GHZ;
      channel = SL_RAIL_SDK_IEEE802154_CHANNEL_2P4;
      break;
    case SL_RAIL_SDK_UTIL_PROTOCOL_IEEE802154_GB868_915MHZ:
      standard_protocol = SL_RAIL_SDK_IEEE802154_STD_IEEE802154G_915MHZ;
      channel = SL_RAIL_SDK_IEEE802154_CHANNEL_915;
      break;
    case SL_RAIL_SDK_UTIL_PROTOCOL_IEEE802154_GB868_863MHZ:
      standard_protocol = SL_RAIL_SDK_IEEE802154_STD_IEEE802154G_863MHZ;
      channel = SL_RAIL_SDK_IEEE802154_CHANNEL_863;
      break;
    default:
      status = SL_RAIL_SDK_IEEE802154_UNSUPPORTED_PROTOCOL;
      break;
  }

  if (status != SL_RAIL_SDK_IEEE802154_OK) {
    return status;
  }

  // sets the PAN ID (by default for broadcast)
  (void)sl_rail_sdk_ieee802154_set_pan_id_filter(r_handle,
                                                 DEFAULT_BROADCAST_PAN_ID);
  // sets the Short address/Source address (by default for Broadcast)
  (void)sl_rail_sdk_ieee802154_set_short_addr_filter(r_handle,
                                                     DEFAULT_BROADCAST_SRC_ADDR);
  // obtains the unique number of the system
#if defined(_SILICON_LABS_32B_SERIES_2)
  system_number = SYSTEM_GetUnique();
#else
  system_number = sl_hal_system_get_unique();
#endif
  // sets the long address
  (void)sl_rail_sdk_ieee802154_set_long_addr_filter(r_handle, &system_number);

  return status;
}

sl_rail_sdk_ieee802154_std_t sl_rail_sdk_ieee802154_get_std(void)
{
  return standard_protocol;
}

uint16_t sl_rail_sdk_ieee802154_get_channel(void)
{
  return channel;
}
...
{
  if (frame == NULL) {
    app_log_error("sl_rail_sdk_ieee802154_print_frame ERR: parameter\n");
    return;
  }
  ...
  if ((std == SL_RAIL_SDK_IEEE802154_STD_IEEE802154G_863MHZ)
    ...
    app_log_info("  CRC length:        %s\n",
                 frame->phr_config & SL_RAIL_SDK_IEEE802154G_PHR_CRC_2_BYTE ? "2B" : "4B");
  }
...
void sl_rail_sdk_ieee802154_print_ack(sl_rail_sdk_ieee802154_std_t std,
                                      const uint8_t *buffer)
{
  uint8_t size = 0U;

  if (buffer == NULL) {
    app_log_error("sl_rail_sdk_ieee802154_print_ack ERR: parameter\n");
    return;
  }

  if (std == SL_RAIL_SDK_IEEE802154_STD_IEEE802154_2P4GHZ) {
    size = SL_RAIL_SDK_IEEE802154_ACK_PACKET_LENGTH;
  } else {
    size = SL_RAIL_SDK_IEEE802154G_ACK_PACKET_LENGTH;
  }
                                            ...
                                            volatile sl_rail_sdk_ieee802154_cli_data *cli_desired)
{
  if (tx_frame == NULL || cli_req == NULL || cli_desired == NULL) {
    app_log_error("sl_rail_sdk_ieee802154_request_manager ERR: parameter\n");
    return;
  }
  ...
  if (cli_req->std_requested) {
    ...
    sl_rail_sdk_ieee802154_change_std(r_handle,
  ...
  if (cli_req->cfgcrc_requested) {
    ...
    if (cli_desired->desired_crc == SL_RAIL_SDK_IEEE802154G_CRC_LENGTH_2BYTE) {
      // 2 bytes
...
void sl_rail_sdk_ieee802154_prepare_sending(sl_rail_sdk_ieee802154_frame_t *tx_frame,
                                            void* payload, uint16_t payload_size)
{
  if (tx_frame == NULL || payload == NULL || payload_size == 0) {
    app_log_error("sl_rail_sdk_ieee802154_prepare_sending ERR: parameter\n");
    return;
  }
...
void sl_rail_sdk_ieee802154_update_status(sl_rail_sdk_ieee802154_status_t *comm_status,
                                          const sl_rail_sdk_ieee802154_frame_t *tx_frame)
{
  if (comm_status == NULL || tx_frame == NULL) {
    app_log_error("sl_rail_sdk_ieee802154_update_status ERR: parameter\n");
    return;
  }

  comm_status->std = sl_rail_sdk_ieee802154_get_std();
  comm_status->ack = sl_rail_sdk_ieee802154_get_ack_status();
  comm_status->channel = sl_rail_sdk_ieee802154_get_channel();
  ...
  if ((comm_status->std == SL_RAIL_SDK_IEEE802154_STD_IEEE802154G_863MHZ)
    ...
    if ((tx_frame->phr_config & SL_RAIL_SDK_IEEE802154G_PHR_DATA_WHITENING_ON)
        == SL_RAIL_SDK_IEEE802154G_PHR_DATA_WHITENING_ON) {
      comm_status->data_whitening = true;
    } else {
      comm_status->data_whitening = false;
    }

    if ((tx_frame->phr_config & SL_RAIL_SDK_IEEE802154G_PHR_CRC_2_BYTE)
        == SL_RAIL_SDK_IEEE802154G_PHR_CRC_2_BYTE) {
      comm_status->crc_length = SL_RAIL_SDK_IEEE802154G_CRC_LENGTH_2BYTE;
    } else {
      comm_status->crc_length = SL_RAIL_SDK_IEEE802154G_CRC_LENGTH_4BYTE;
    }
  }
}

bool sl_rail_sdk_ieee802154_is_change_requested(
  volatile sl_rail_sdk_ieee802154_cli_requests *cli_requests)
{
  if (cli_requests == NULL) {
    app_log_error("sl_rail_sdk_ieee802154_is_change_requested ERR: parameter\n");
    return false;
  }
...
int16_t sl_rail_sdk_ieee802154_pack_data_frame(sl_rail_sdk_ieee802154_std_t std,
                                               sl_rail_sdk_ieee802154_frame_t *frame,
                                               uint16_t *frame_size,
                                               uint8_t *frame_buffer)
{
  ...
  if ((frame == NULL) || (frame_size == NULL) || (frame_buffer == NULL)) {
    app_log_error("sl_rail_sdk_ieee802154_pack_data_frame ERR: parameter");
  ...
  switch (std) {
    ...
    case SL_RAIL_SDK_IEEE802154_STD_IEEE802154G_915MHZ:
      ...
      break;
    default:
      ret_val = SL_RAIL_SDK_IEEE802154_UNSUPPORTED_PROTOCOL;
      break;
  }
...
int16_t sl_rail_sdk_ieee802154_unpack_data_frame(sl_rail_sdk_ieee802154_std_t std,
                                                 sl_rail_sdk_ieee802154_frame_t *frame,
                                                 uint8_t *frame_buffer,
                                                 uint16_t max_frame_size)
{
  ...
  if ((frame == NULL) || (frame_buffer == NULL)) {
    app_log_error("sl_rail_sdk_ieee802154_unpack_data_frame ERR: parameter");
    return SL_RAIL_SDK_IEEE802154_ERROR;
  }
  ...
  switch (std) {
    case SL_RAIL_SDK_IEEE802154_STD_IEEE802154_2P4GHZ:
      ...
      max_payload_size = max_frame_size
                         - SL_RAIL_SDK_IEEE802154_PHR_LENGTH
                         - SL_RAIL_SDK_IEEE802154_MHR_LENGTH
                         - SL_RAIL_SDK_IEEE802154_CRC_LENGTH;
    ...
    case SL_RAIL_SDK_IEEE802154_STD_IEEE802154G_915MHZ:
      ...
      if (frame->phr_config & SL_RAIL_SDK_IEEE802154G_PHR_CRC_2_BYTE) {
        max_payload_size = max_frame_size
                           - SL_RAIL_SDK_IEEE802154G_PHR_LENGTH
                           - SL_RAIL_SDK_IEEE802154_MHR_LENGTH
                           - SL_RAIL_SDK_IEEE802154G_CRC_LENGTH_2BYTE;
      } else {
        max_payload_size = max_frame_size
                           - SL_RAIL_SDK_IEEE802154G_PHR_LENGTH
                           - SL_RAIL_SDK_IEEE802154_MHR_LENGTH
                           - SL_RAIL_SDK_IEEE802154G_CRC_LENGTH_4BYTE;
      }
                                              ...
                                              uint16_t *new_channel)
{
  ...
  *current_ack = false;
  ...
  if ((*current_std == SL_RAIL_SDK_IEEE802154_STD_IEEE802154G_863MHZ)
      || (*current_std == SL_RAIL_SDK_IEEE802154_STD_IEEE802154G_915MHZ)) {
    // release PHR config
  ...
  switch (*current_std) {
    case SL_RAIL_SDK_IEEE802154_STD_IEEE802154_2P4GHZ:
      *new_channel = SL_RAIL_SDK_IEEE802154_CHANNEL_2P4;
      break;
    case SL_RAIL_SDK_IEEE802154_STD_IEEE802154G_863MHZ:
      *new_channel = SL_RAIL_SDK_IEEE802154_CHANNEL_863;
      break;
    case SL_RAIL_SDK_IEEE802154_STD_IEEE802154G_915MHZ:
      *new_channel = SL_RAIL_SDK_IEEE802154_CHANNEL_915;
      break;
    default:
      break;
  }
  ...
  (void)sl_rail_sdk_ieee802154_reinit(rail_handle, *current_std);
  // sets the previous PAN ID
  (void)sl_rail_sdk_ieee802154_set_pan_id_filter(rail_handle,
                                                 tx_frame->mhr_config.destination_pan_id);
  // sets the previous short address
  (void)sl_rail_sdk_ieee802154_set_short_addr_filter(rail_handle,
                                                     tx_frame->mhr_config.source_address);
  // sets the long address
  (void)sl_rail_sdk_ieee802154_set_long_addr_filter(rail_handle, &system_number);
}
                                             ...
                                             sl_rail_sdk_ieee802154_std_t std)
{
  ...
  return SL_RAIL_SDK_IEEE802154_OK;
}
