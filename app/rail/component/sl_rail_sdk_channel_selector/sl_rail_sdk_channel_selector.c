/***************************************************************************//**
 * @file sl_rail_sdk_channel_selector.c
...
#endif
#include "sl_rail_sdk_channel_selector_config.h"
...
{
#ifdef SL_CATALOG_RAIL_SDK_PHY_SELECTOR_PRESENT
  uint8_t selected_phy = get_selected_phy();
#else
  uint8_t selected_phy = 0;
#endif
  uint16_t start = channelConfigs[selected_phy]->configs[0U].channelNumberStart;
  uint16_t end = channelConfigs[selected_phy]->configs[0U].channelNumberEnd;
...
void set_configured_channel(void)
{
  ...
  if (!success) {
    set_selected_channel(channelConfigs[selected_phy]->configs[0U].channelNumberStart);
    #if defined(SL_CATALOG_APP_LOG_PRESENT)
