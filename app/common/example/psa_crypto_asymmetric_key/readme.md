...
5. The `PSA_KEY_USAGE_COPY` policy does not apply to the wrapped key.
...
### Key ID Change
Change the `PERSISTENT_KEY_ID` and `PERSISTENT_COPY_KEY_ID` values in `app_process.h` if these key IDs had already existed in NVM3.
