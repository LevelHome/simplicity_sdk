...
#define SL_WISUN_NWM_REMOTE_CTRL_RESP_BUFF_ERROR_STR              "[Invalid result buffer]"
...
/// Response buffer no test result
...
static sl_wisun_coap_packet_t * _parse_ping(const sl_wisun_coap_packet_t * const req_packet)
{
  ...
  if (ping_cmd.arg3 != NULL && !strncmp(ping_cmd.arg3,
    ...
    if (ping_cmd.arg3 != NULL && inet_pton(AF_INET6, ping_cmd.arg3, &remote_addr.sin6_addr) != 1) {
      sl_wisun_coap_destroy_payload_str(req_payload);
...
static sl_wisun_coap_packet_t * _parse_iperf(const sl_wisun_coap_packet_t * const req_packet)
{
  ...
  if (!strncmp(iperf_cmd.arg1,
    ...
    if (get_res_type != SL_WISUN_NWM_REMOTE_CTRL_BUF_TARGET_NONE) {
      ...
      if (resp_packet == NULL || resp_packet->payload_ptr == NULL || resp_packet->payload_len == 0) {
...
static sl_wisun_coap_packet_t * _build_response(const sl_wisun_coap_packet_t * const req_packet,
                                                const sl_wisun_nwm_remote_ctrl_buf_target_t target)
{
  sl_wisun_coap_packet_t *resp_coap_packet = NULL;
  ...
  resp_coap_packet = _build_response_packet(&resp, req_packet);
  if (resp_coap_packet == NULL) {
    return NULL;
  }

  // Fill coap response packet content format
  switch (target) {
    case SL_WISUN_NWM_REMOTE_CTRL_BUF_TARGET_NONE:
      resp_coap_packet->content_format = COAP_CT_NONE;
      break;

    case SL_WISUN_NWM_REMOTE_CTRL_BUF_TARGET_BORDER_ROUTER:
      resp_coap_packet->content_format = COAP_CT_JSON;
      break;

    case SL_WISUN_NWM_REMOTE_CTRL_BUF_TARGET_PARENT:
      resp_coap_packet->content_format = COAP_CT_JSON;
      break;

    case SL_WISUN_NWM_REMOTE_CTRL_BUF_TARGET_ALL:
      resp_coap_packet->content_format = COAP_CT_JSON;
      break;

    case SL_WISUN_NWM_REMOTE_CTRL_BUF_TARGET_SINGLE:
      resp_coap_packet->content_format = COAP_CT_JSON;
      break;

    case SL_WISUN_NWM_REMOTE_CTRL_BUF_TARGET_IPERF_JSON:
      resp_coap_packet->content_format = COAP_CT_JSON;
      break;

    case SL_WISUN_NWM_REMOTE_CTRL_BUF_TARGET_IPERF_TEXT:
      resp_coap_packet->content_format = COAP_CT_TEXT_PLAIN;
      break;

    case SL_WISUN_NWM_REMOTE_CTRL_BUF_TARGET_NBINF:
      resp_coap_packet->content_format = COAP_CT_JSON;
      break;
