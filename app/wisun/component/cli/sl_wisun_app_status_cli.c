/***************************************************************************//**
 * @file sl_wisun_app_status_cli.c
 * @brief Wi-SUN Application Status CLI handler
 *******************************************************************************
 * # License
...
static char _ip_buff[IPV6_STRING_SIZE] = { 0 };
 ...
 *****************************************************************************/
void sl_app_status_cli_get_address(const sl_cli_command_arg_t *arguments)
{
  (void)arguments;

  const sockaddr_in6_t* address = sl_wisun_app_status_get_remote_address();
  inet_ntop(AF_INET6, &(address->sin6_addr), _ip_buff, sizeof(_ip_buff));
  printf("remote_address=%s\n", _ip_buff);
  printf("remote_port=%d\n", ntohs(address->sin6_port));
  memset(_ip_buff, 0, sizeof(_ip_buff));
}
 ...
 * @param[in] arguments Arguments
 *****************************************************************************/
void sl_app_status_cli_get_schedule(const sl_cli_command_arg_t *arguments)
{
  (void)arguments;
 ...
 * @param[in] arguments Arguments
 *****************************************************************************/
void sl_app_status_cli_set_schedule(const sl_cli_command_arg_t *arguments)
{
 ...
 * @param[in] arguments Arguments
 *****************************************************************************/
void sl_app_status_cli_set_address(const sl_cli_command_arg_t *arguments)
{
  uint16_t port = 0;
  char *address = NULL;
