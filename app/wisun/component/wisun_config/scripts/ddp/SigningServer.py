...
serial_cmd = "-create_serial"
if sys.platform.startswith('win'):
  serial_cmd = "-rand_serial"
  jinja2_path = os.path.dirname(jinja2.__file__)
  openssl_path = os.path.abspath(os.path.join(jinja2_path, '../../../../cpms/applications/'))
  if os.path.exists(openssl_path):
    if openssl_path not in sys.path:
...
def generate_root(config: str, oem_name: str, oem_country: str, output_key: str, output_cert: str):
  ...
  cmd = f'openssl ca -batch -selfsign {serial_cmd} -keyfile {output_key} -in {req} -out {output_cert} -notext -extensions v3_root -policy wisun_ca_policy -config {config} -subj \"{dn}\"'
  try:
    _process(cmd)
...
def generate_batch(config: str, oem_name: str, oem_country: str, batch: str, sign_cert: str, sign_key: str, output_key: str, output_cert: str):
  ...
  req = generate_req(config, output_key)
  cmd = f'openssl ca -batch {serial_cmd} -cert {sign_cert} -keyfile {sign_key} -in {req} -out {output_cert} -notext -extensions v3_batch -policy wisun_ca_policy -config {config} -subj \"{dn}\"'
  try:
    _process(cmd)
  except Exception as e:
    raise e
...
def generate_device(config: str, oid: str, sn: str, csr: bytes, sign_cert: str, sign_key: str, output_cert: str):
  ...
  cmd = f'openssl ca -batch {serial_cmd} -cert {sign_cert} -keyfile {sign_key} -in {req} -out {output_cert} -notext -extfile {ext} -extensions v3_device -policy wisun_device_policy -config {config}'
  try:
    result = _process(cmd)
  except Exception as e:
    raise e
...
def GetCerts(csr: bytes, sn: str, config: str) -> Tuple[bytes, bytes, bytes]:
  ...
  """
  oid = load_oid()
  logger.info(f"Utilizing device OID {oid}")
  logger.info(f"Generating local device certificate for {sn}")
...
def _setup_certs(args):
  return SetupCerts(args.co, args.cu, args.oid, args.config)
