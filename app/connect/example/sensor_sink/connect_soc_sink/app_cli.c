...
void cli_form(sl_cli_command_arg_t *arguments)
{
  ...
  parameters.radioChannel = sl_cli_get_argument_uint16(arguments, 0);
...
void cli_pjoin(sl_cli_command_arg_t *arguments)
{
  ...
  if (sl_cli_get_argument_count(arguments) > 1) {
    ...
    if (status != EMBER_SUCCESS) {
      app_log_info("Set selective join payload status: 0x%02X", status);
    }
  } else {
...
void cli_unset_security_key(sl_cli_command_arg_t *arguments)
{
  (void)arguments;
  #ifdef SL_CATALOG_CONNECT_AES_SECURITY_PRESENT
// Only remove connect stack pointer
...
bool set_security_key(uint8_t* key, size_t key_length)
{
  ...
  // destroy the key if exists - it always have to be exist, as it is created in
  // the init function if not exists at that point
