...
static bool set_security_key(const uint8_t* key, size_t key_length);
...
void cli_commission(sl_cli_command_arg_t *arguments)
{
  ...
  uint16_t channel = sl_cli_get_argument_uint16(arguments, 2);
...
static EmberStatus send_tx_test_packet(void)
{
  uint8_t tx_test_packet[SL_CONNECT_BUFFER_SIZE];
