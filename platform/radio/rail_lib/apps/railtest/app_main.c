...
RAIL_Time_t txStartTime = 0U;
RAIL_Time_t txScheduledTime = 0U;
...
RAIL_HFXOCompensationConfig_t compensationConfig = {
...
void sl_rail_test_internal_app_init(void)
{
  ...
  unsigned int seed;
  (void) RAIL_GetRadioEntropy(railHandle, (uint8_t *) &seed, sizeof(seed));
  srand(seed);
}
...
void sl_rail_util_on_event(RAIL_Handle_t railHandle, RAIL_Events_t events)
{
  ...
  if (enableRxPacketEventTimeCapture
    ...
    sl_rac_info_start.radioStateTimerTick = RAIL_GetTimerTick(RAIL_TIMER_TICK_DEFAULT);
...
void processPendingCalibrations(void)
{
  ...
  if (calibrateRadio && (calsInMode | calsInTxMode) && !skipCalibrations && !receivingPacket) {
    ...
    if (status == RAIL_STATUS_NO_ERROR) {
      ...
      if ((pendingCals & RAIL_CAL_COMPENSATE_HFXO) == 0U) {
    ...
    if (pendingCals & RAIL_CAL_COMPENSATE_HFXO) {
      ...
      if ((radioState == RAIL_RF_STATE_RX) || (radioState == RAIL_RF_STATE_IDLE)) {
