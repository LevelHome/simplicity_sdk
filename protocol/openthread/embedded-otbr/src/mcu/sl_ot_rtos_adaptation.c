...
#if defined(SL_CATALOG_OPENTHREAD_BORDER_ROUTER_PRESENT) && defined(SL_CATALOG_KERNEL_PRESENT)
#define STACK_TASK_ENABLED (0)
#elif defined(SL_CATALOG_KERNEL_PRESENT)
#define STACK_TASK_ENABLED (1)
#else
#define STACK_TASK_ENABLED (0)
#endif
...
#endif /*STACK_TASK_ENABLED*/

/* OT uart task specific settings */
static sl_ot_rtos_thread_t sli_ot_uart_task_info;
__ALIGNED(8) static uint8_t sli_ot_uart_task_mem[SL_OPENTHREAD_STACK_TASK_MEM_SIZE];
__ALIGNED(4) static uint8_t sli_ot_uart_task_cb[osThreadCbSize];
static void sli_ot_uart_task(void *aContext);

static const osThreadAttr_t sli_ot_uart_task_attr = {
    .name       = "OT UART",
    .attr_bits  = 0u,
    .stack_size = SL_OPENTHREAD_STACK_TASK_MEM_SIZE,
    .stack_mem  = sli_ot_uart_task_mem,
    .cb_mem     = sli_ot_uart_task_cb,
    .cb_size    = osThreadCbSize,
    .priority   = (osPriority_t)SL_OPENTHREAD_RTOS_STACK_TASK_PRIORITY + 1,
};

static const osSemaphoreAttr_t sli_ot_uart_semaphore_attr = {.name = "OT UART Semaphore"};
...
__ALIGNED(8) static uint8_t sli_ot_cli_task_mem[SL_MCU_OPENTHREAD_CLI_TASK_MEM_SIZE];
...
static const osThreadAttr_t sli_ot_cli_task_attr = {
    .name       = "OT CLI",
    .attr_bits  = 0u,
    .stack_size = SL_MCU_OPENTHREAD_CLI_TASK_MEM_SIZE,
...
// osMutexId_t sli_ot_uart_mutex;
// static const osMutexAttr_t sli_uart_mutex_attributes = {
//   .name = "OT UART Mutex",
//   .attr_bits = osMutexRecursive | osMutexPrioInherit,
// };
...
#if CLI_TASK_ENABLED
extern void app_cli_process_action();
#endif
...
void sl_ot_rtos_uart_init(void)
{
    // Create main uart task for operation..
    sl_ot_rtos_stack_create(&sli_ot_uart_task_info,
                            sli_ot_uart_task,
                            &sli_ot_uart_task_attr,
                            &sli_ot_uart_semaphore_attr);

    // sli_ot_uart_mutex = osMutexNew(&sli_uart_mutex_attributes);
    // EFM_ASSERT(sli_ot_uart_mutex != NULL);
}

// Initialize the stack task.
void sl_ot_rtos_stack_init(void)
{
    sl_ot_rtos_uart_init();
...
{
#if CLI_TASK_ENABLED
    if (event & SL_OT_RTOS_EVENT_UART_CLI)
    {
        // If SL_OT_RTOS_EVENT_UART_CLI is raised, set the event flag for CLI task.
    ...
    if (event & SL_OT_RTOS_EVENT_UART)
    {
        ...
        sl_ot_rtos_set_event_flag(&sli_ot_uart_task_info, event);
#if STACK_TASK_ENABLED
        sl_ot_rtos_set_event_flag(&sli_ot_stack_task_info, event);
#endif /*STACK_TASK_ENABLED*/
    }
...
static void sli_ot_stack_task(void *context)
{
    ...
    while (!otSysPseudoResetWasRequested())
    {
        ...
        app_process_action(); // TODO Will be optimizied
...
static void sli_ot_cli_task(void *aContext)
{
    ...
    while (1)
    {
        ...
        // sl_ot_rtos_acquire_stack_mutex();
        app_cli_process_action();
        // sl_ot_rtos_release_stack_mutex();

        sli_ot_cli_task_info.active_duration += otPlatTimeGet() - timestamp;
        // Wait for the CLI event..
        (void)sl_ot_rtos_wait_for_event(&sli_ot_cli_task_info, SL_OT_RTOS_EVENT_UART_CLI);
    }
}
#endif
...
static void sli_ot_uart_task(void *aContext)
{
    ...
    while (1)
    {
        ...
        sli_ot_uart_task_info.active_duration += otPlatTimeGet() - timestamp;
        // Wait for the CLI event..
        (void)sl_ot_rtos_wait_for_event(&sli_ot_uart_task_info, SL_OT_RTOS_EVENT_UART);
    }
}
