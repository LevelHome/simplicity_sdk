...
#define GTK_DEFAULT_LIFETIME_S      (60 * 60 * 24 * 30)     // 30 days
#define GTK_EXPIRE_MISMATCH_TIME_S   60                     // Supplicant GTK expire time mismatch occurs if GTK expires before this time
...
#define LGTK_DEFAULT_LIFETIME_S     (60 * 60 * 24 * 90)     // 90 days

#define LGTK_ALL_HASHES_LEN          GTK_HASH_LEN * LGTK_NUM

#define ON_GTK                       0
#define ON_LGTK                      1

#define LGTK_OFFSET                  4
...
#define NODE_ROLE_UNDEFINED            0xFF               // Corresponds to a FAN1.0 router

typedef struct {
    uint8_t                key[GTK_LEN];              /**< Group Transient Key (128 bits) */
    uint8_t                hash[GTK_HASH_LEN];        /**< GTK hash */
    struct {
        uint32_t           key_id;                    /**< Key ID for GAK */
    } gak;
    uint64_t               expirytime;                /**< GTK expiry time on storage */
    sli_wisun_timer_t      lifetime_timer;            /**< Timer for GTK lifetime */
    ...
    uint8_t                index;                     /**< Index of the key in parent structure */
} sec_prot_gtk_t;

typedef struct sec_prot_gtk_keys_s {
    sec_prot_gtk_t         gtk[GTK_NUM];              /**< 4 (LFN) Group Transient Keys */
    bool                   updated: 1;                /**< (LFN) Group Transient Keys has been updated */
    bool                   is_lgtk: 1;                /**< If true, treat the above as LGTK info */
} sec_prot_gtk_keys_t;
...
typedef struct {
    uint8_t                key[PMK_LEN];              /**< Pairwise Master Key (256 bits) */
    uint32_t               key_id;                    /**< Pairwise Master Key */
    uint8_t                pmkid[PMKID_LEN];          /**< PMKID */
    uint64_t               replay_cnt;                /**< Pairwise Master Key replay counter */
    sli_wisun_timer_t      lifetime_timer;            /**< Timer for PMK lifetime */
    bool                   set: 1;                    /**< Pairwise Master Key set */
    bool                   replay_cnt_set: 1;         /**< Pairwise Master Key replay counter set */
} sec_prot_pmk_t;

typedef struct {
    uint8_t                ptkid[PTKID_LEN];          /**< PTKID */
    struct {
        uint8_t            key[KCK_LEN];              /**< Key Confirmation Key  */
        uint32_t           key_id;                    /**< Key ID for KCK */
    } kck;
    struct {
        uint8_t            key[KEK_LEN];              /**< Key Encryption Key  */
        uint32_t           key_id;                    /**< Key ID for KEK */
    } kek;
    struct {
        uint8_t            key[TK_LEN];               /**< Temporal Key */
        uint32_t           key_id;                    /**< Key ID for TK */
    } tk;
    sli_wisun_timer_t      lifetime_timer;            /**< Timer for PTK lifetime */
    bool                   set: 1;                    /**< Pairwise Transient Key set */
} sec_prot_ptk_t;

// Security key data
typedef struct {
    sec_prot_pmk_t         pmk;                       /**< Pairwise Master Key */
    sec_prot_ptk_t         ptk;                       /**< Pairwise Transient Key */
    ...
    sec_prot_gtk_keys_t    *lgtks;                    /**< LFN Group Transient Keys */
    const sec_prot_certs_t *certs;                    /**< Certificates */
    uint8_t                gtkl;                      /**< Remote GTKL information */
    uint8_t                lgtkl;                     /**< Remote LGTKL information */
    int8_t                 gtk_set_index;             /**< Index of GTK to set */
    int8_t                 lgtk_set_index;            /**< Index of LGTK to set */
...
// Frame counter data
typedef struct {
    uint8_t gtkhash[GTK_HASH_LEN];                    /**< GTK hash */
...
typedef struct {
    ...
    uint8_t                pmkid[PMKID_LEN];          /**< Pairwise Master Key ID */
    uint8_t                ptkid[PTKID_LEN];          /**< Pairwise Transient Key ID */
    uint8_t                kck[KCK_LEN];              /**< Key Confirmation Key */
    uint8_t                kek[KEK_LEN];              /**< Key Encryption Key */
    uint8_t                ptk_eui_64[8];             /**< Remote EUI-64 used to derive PTK or NULL */
    sec_prot_gtk_hash_t    ins_gtk_hash[GTK_NUM];     /**< Hashes for inserted GTKs for a PTK */
...
typedef struct sec_prot_keys_nw_info_s {
    uint8_t network_name[32];                              /**< Network name for keys */
    uint8_t network_name_length;                           /**< Network name length in bytes */
    ...
    uint16_t lfn_version;                                  /**< LFN version for keys */
...
/**
 * sec_prot_keys_gtks_init initialises (L)GTK keys
 *
 * \param gtks (L)GTK keys
 * \param is_lgtk True to init as LGTK keys, false for GTK
 * \param callback function pointer for GTK expiration callback
 *
 */
void sec_prot_keys_gtks_init(sec_prot_gtk_keys_t *gtks, bool is_lgtk, void (*callback)(sli_wisun_timer_t *));
...
/**
 * sec_prot_keys_pmk_set set Pairwise Master Key
 *
 * \param sec_keys security keys
 ...
 *
 */
void sec_prot_keys_pmk_set(sec_prot_keys_t *sec_keys, const uint8_t *key, uint32_t key_id, const uint8_t *pmkid, uint32_t lifetime);

/**
 * sec_prot_keys_pmk_delete resets PMK
 *
 * \param pmk PMK
 *
 */
void sec_prot_keys_pmk_reset(sec_prot_pmk_t *pmk);
 ...
 *
 */
sec_prot_pmk_t *sec_prot_keys_pmk_get(sec_prot_keys_t *sec_keys);
...
/**
 * sec_prot_keys_ptk_set writes Pairwise Transient Key
 *
 * \param sec_keys security keys
 * \param kck_key KCK
 * \param kck_key_id Key id for KCK
 * \param kek_key KEK
 * \param kek_key_id Key id for KEK
 * \param tk_key TK
 * \param tk_key_id Key id for TK
 * \param ptkid PTKID
 * \param lifetime PTK lifetime
 *
 */
void sec_prot_keys_ptk_set(sec_prot_keys_t *sec_keys, uint8_t *kck_key, uint32_t kck_key_id,
                           uint8_t *kek_key, uint32_t kek_key_id, uint8_t *tk_key, uint32_t tk_key_id,
                           const uint8_t *ptkid, uint32_t lifetime);

/**
 * sec_prot_keys_ptk_reset resets PTK
 *
 * \param ptk PTK
 *
 */
void sec_prot_keys_ptk_reset(sec_prot_ptk_t *ptk);
 ...
 *
 */
sec_prot_ptk_t *sec_prot_keys_ptk_get(sec_prot_keys_t *sec_keys);
...
/**
 * sec_prot_keys_gtkl_gtk_is_live checks whether GTK is live on GTK liveness storage
 *
 * \param sec_keys security keys
 * \param index index of the GTK which liveness is returned
 * \param on_lgtk True to perform on LGTK, false on GTK
 *
 * \return TRUE GTK is live, FALSE GTK is not live
 *
 */
bool sec_prot_keys_gtkl_gtk_is_live(sec_prot_keys_t *sec_keys, uint8_t index, bool on_lgtk);
 ...
 *
 */
int8_t sec_prot_keys_gtkl_gtk_live_set(sec_prot_keys_t *sec_keys, uint8_t index, bool on_lgtk);
 ...
 *
 */
int8_t sec_prot_keys_gtk_insert_index_set(sec_prot_keys_t *sec_keys, uint8_t index, bool on_lgtk);
 ...
 *
 */
void sec_prot_keys_gtkl_from_gtk_insert_index_set(sec_prot_keys_t *sec_keys, bool on_lgtk);
 ...
 *
 */
int8_t sec_prot_keys_gtk_insert_index_from_gtkl_get(sec_prot_keys_t *sec_keys, bool on_lgtk);
 ...
 *
 */
uint8_t *sec_prot_keys_get_gtk_to_insert(sec_prot_keys_t *sec_keys, uint8_t *index, bool on_lgtk);
 ...
 * \param index index
 * \param gtk gtk value
 * \param gtkhash GTK hash
 * \param gak_key_id ID of GAK
 ...
 *
 */
int8_t sec_prot_keys_gtk_set(sec_prot_gtk_keys_t *gtks, uint8_t index, const uint8_t *gtk, const uint8_t *gtkhash, uint32_t gak_key_id, uint32_t lifetime);
 ...
 *
 */
sec_prot_gtk_t *sec_prot_keys_gtk_get(sec_prot_gtk_keys_t *gtks, uint8_t index);
...
/**
 * sec_prot_keys_gtk_lifetime_update decrements GTK lifetime
 *
 * \param gtks GTK keys
 * \param index index for GTK
 * \param seconds new lifetime
 *
 */
void sec_prot_keys_gtk_lifetime_update(sec_prot_gtk_keys_t *gtks, uint8_t index, uint16_t seconds);

/**
 * sec_prot_keys_check_expiry_mismatch Detect key expiry time mismatch
 *
 * \param gtks GTK keys
 * \param index index for GTK
 ...
 *
 */
void sec_prot_keys_check_expiry_mismatch(sec_prot_gtk_keys_t *gtks, uint8_t index, uint64_t current_time);
 ...
 *
 * \param gtks GTK keys
 * \param gtkhash GTK hash to validate
 * \param key_index returns matching GTK hash index if found, -1 otherwise
 *
 * \return < 0 failure
 * \return >= 0 success
 */
int8_t sec_prot_keys_gtk_valid_check(sec_prot_gtk_keys_t *gtks, const uint8_t *gtkhash, int8_t *key_index);
