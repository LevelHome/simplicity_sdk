...
#define GSME_ENDPOINT SL_ZIGBEE_AF_PLUGIN_GBCS_GAS_METER_GSME_ENDPOINT
// This is the remote endpoint representing the Remote CommsHub which is part of the CHF.
#define CHF_ENDPOINT  SL_ZIGBEE_AF_PLUGIN_GBCS_GAS_METER_CHF_ENDPOINT
// Offset from the begin of a tunneled remote party message where the GBZ packet begins
#define GBZ_OFFSET SL_ZIGBEE_AF_PLUGIN_GBCS_GAS_METER_GBZ_OFFSET
// The GBCS spec recommends this value at 30 minutes (1800 seconds).
// For testing purposes we default to 20 seconds.
#define MIRROR_UPDATE_INTERVAL_SECONDS SL_ZIGBEE_AF_PLUGIN_GBCS_GAS_METER_REPORT_INTERVAL
...
// This event replaces the use of the sl_zigbee_af_main_tick_cb which should
// not be used by sleepy devices as it does not properly allow for power
// management.
sl_zigbee_af_event_t sleepyMeterEvent;
static void sleepyMeterEventHandler(sl_zigbee_af_event_t * event);
...
typedef struct {
  ...
  sl_zigbee_af_attribute_id_t *attributeIds;
} sli_zigbee_af_sgsme_attribute_data;

static sl_zigbee_af_attribute_id_t basicClusterAttributes[] = {
...
static sl_zigbee_af_attribute_id_t meteringClusterAttributes[] = {
...
static sl_zigbee_af_attribute_id_t prepaymentClusterAttributes[] = {
...
{
  uint8_t i;
  uint8_t reportingStatus = SL_ZIGBEE_ZCL_ATTRIBUTE_REPORTING_STATUS_ATTRIBUTE_REPORTING_COMPLETE;

  if (state == MIRROR_READY) {
    for (i = 0; i < GAS_METER_REPORT_LIST_COUNT; i++) {
      reportList[i].report = true;
    }
    sl_zigbee_af_write_server_attribute(GSME_ENDPOINT,
 ...
 *
 */
void sl_zigbee_af_gbcs_gas_meter_init_cb(uint8_t init_level)
{
  switch (init_level) {
    case SL_ZIGBEE_INIT_LEVEL_EVENT:
    {
      sl_zigbee_af_event_init(&sleepyMeterEvent,
...
// *
// *******************************************************************
void sl_zigbee_af_cluster_init_cb(uint8_t endpointId,
                                  uint16_t clusterId)
{
  uint8_t deviceType = SL_ZIGBEE_ZCL_METERING_DEVICE_TYPE_GAS_METERING;
  if ((endpointId == GSME_ENDPOINT)
      && (clusterId == ZCL_SIMPLE_METERING_CLUSTER_ID)) {
    (void) sl_zigbee_af_write_attribute(endpointId,
                                        ZCL_SIMPLE_METERING_CLUSTER_ID,
                                        ZCL_METERING_DEVICE_TYPE_ATTRIBUTE_ID,
                                        CLUSTER_MASK_SERVER,
                                        (uint8_t*) &deviceType,
...
{
  sl_status_t status;
  sl_802154_short_addr_t partnerShortId = 0x0000; // Node ID of trust center which is the Comms Hub
  ...
  sl_zigbee_af_event_set_delay_ms(&sleepyMeterEvent,
  ...
  switch (state) {
    ...
    case REGISTRATION_COMPLETE:
      ...
      if (SL_STATUS_OK == sl_zigbee_lookup_eui64_by_node_id(partnerShortId, partnerEui)
          && sl_zigbee_af_gbcs_device_log_exists(partnerEui, SL_ZIGBEE_AF_GBCS_CHF_DEVICE_TYPE)) {
      ...
      // support mirroring.
      status = sl_zigbee_af_find_devices_by_profile_and_cluster(SL_ZIGBEE_RX_ON_WHEN_IDLE_BROADCAST_ADDRESS,
                                                                SE_PROFILE_ID,
                                                                ZCL_SIMPLE_METERING_CLUSTER_ID,
    ...
    case MIRROR_UPDATE_IN_PROGRESS:
      while (clusterToReport < GAS_METER_REPORT_LIST_COUNT) {
        if (reportList[clusterToReport].report) {
          ...
          while (attributeToReport < reportList[clusterToReport].attributeCount) {
            sl_zigbee_af_status_t status = sl_zigbee_af_append_attribute_report_fields(GSME_ENDPOINT,
          ...
          sl_zigbee_af_fill_command_global_server_to_client_report_attributes(reportList[clusterToReport].clusterId,
                                                                              data,
                                                                              bufIndex);
          sl_zigbee_af_set_command_endpoints(GSME_ENDPOINT, mirrorEndpoint);
          sl_zigbee_af_send_command_unicast(SL_ZIGBEE_OUTGOING_DIRECT, mirrorAddress);
      ...
      if (clusterToReport < GAS_METER_REPORT_LIST_COUNT) {
        ...
        uint8_t reportingStatus = SL_ZIGBEE_ZCL_ATTRIBUTE_REPORTING_STATUS_PENDING;
                                              ...
                                              uint16_t bufLen)
{
  ...
  if ((state == SERVICE_DISCOVERY_STARTED || state == MIRROR_CONFIGURATION_STARTED)
    ...
    if ((sl_zigbee_af_get_int16u(buffer, 0, bufLen)
         == ZCL_PHYSICAL_ENVIRONMENT_ATTRIBUTE_ID)
        && (sl_zigbee_af_get_int8u(buffer, 2, bufLen) == 0)
        ...
        // check.
        /* && (sl_zigbee_af_get_int8u(buffer, 4, bufLen) & 0x01) */) {
      sl_zigbee_af_cluster_command_t *currentCommand = sl_zigbee_af_current_command();
      sl_zigbee_af_fill_command_simple_metering_cluster_request_mirror();
      sl_zigbee_af_set_command_endpoints(GSME_ENDPOINT,
                                         currentCommand->apsFrame->sourceEndpoint);
      sl_zigbee_af_send_command_unicast(SL_ZIGBEE_OUTGOING_DIRECT, currentCommand->source);
...
void sl_zigbee_af_simple_metering_server_process_notification_flags_cb(sl_zigbee_af_attribute_id_t attributeId,
                                                                       uint32_t bitMap)
{
  ...
  if (state == MIRROR_READY || state == MIRROR_UPDATE_IN_PROGRESS) {
    if (attributeId == ZCL_NOTIFICATION_FLAGS_4_ATTRIBUTE_ID) {
      requestMessages = bitMap & (SL_ZIGBEE_AF_METERING_NF4_GET_PREPAY_SNAPSHOT | SL_ZIGBEE_AF_METERING_NF4_GET_TOP_UP_LOG | SL_ZIGBEE_AF_METERING_NF4_GET_DEBT_REPAYMENT_LOG);
    } else if (attributeId == ZCL_FUNCTIONAL_NOTIFICATION_FLAGS_ATTRIBUTE_ID) {
      if (bitMap & SL_ZIGBEE_AF_METERING_FNF_NEW_OTA_FIRMWARE) {
        // TODO
      }
      if (bitMap & SL_ZIGBEE_AF_METERING_FNF_CBKE_UPDATE_REQUEST) {
        // TODO
      }
      if (bitMap & (SL_ZIGBEE_AF_METERING_FNF_STAY_AWAKE_REQUEST_HAN | SL_ZIGBEE_AF_METERING_FNF_STAY_AWAKE_REQUEST_WAN)) {
        // TODO
      }
      if (bitMap & SL_ZIGBEE_AF_METERING_FNF_PUSH_HISTORICAL_METERING_DATA_ATTRIBUTE_SET) {
      ...
      if (bitMap & SL_ZIGBEE_AF_METERING_FNF_PUSH_HISTORICAL_PREPAYMENT_DATA_ATTRIBUTE_SET) {
      ...
      if (bitMap & SL_ZIGBEE_AF_METERING_FNF_PUSH_ALL_STATIC_DATA_BASIC_CLUSTER) {
        reportList[GAS_METER_REPORT_LIST_BASIC_CLUSTER_INDEX].report = true;
        setSleepyMeterState(MIRROR_UPDATE_IN_PROGRESS);
      }
      if (bitMap & SL_ZIGBEE_AF_METERING_FNF_PUSH_ALL_STATIC_DATA_METERING_CLUSTER) {
        reportList[GAS_METER_REPORT_LIST_METERING_CLUSTER_INDEX].report = true;
        setSleepyMeterState(MIRROR_UPDATE_IN_PROGRESS);
      }
      if (bitMap & SL_ZIGBEE_AF_METERING_FNF_PUSH_ALL_STATIC_DATA_PREPAYMENT_CLUSTER) {
        reportList[GAS_METER_REPORT_LIST_PREPAYMENT_CLUSTER_INDEX].report = true;
        setSleepyMeterState(MIRROR_UPDATE_IN_PROGRESS);
      }
      if (bitMap & SL_ZIGBEE_AF_METERING_FNF_NETWORK_KEY_ACTIVE) {
        // TODO
      }
      if (bitMap & SL_ZIGBEE_AF_METERING_FNF_TUNNEL_MESSAGE_PENDING) {
                                                    ...
                                                    uint16_t maximumIncomingTransferSize)
{
  ...
  if (tunnelStatus == SL_ZIGBEE_AF_PLUGIN_TUNNELING_CLIENT_SUCCESS) {
    ...
    if (maximumIncomingTransferSize < 1500) {
      sl_zigbee_af_app_println("Warning: tunnel opened but MaximumIncomingTransferSize of server is %d but should be 1500",
...
{
  sl_zigbee_af_gbcs_gas_meter_print("GSME: DeviceLogDeviceRemoved ");
  sl_zigbee_af_gbcs_gas_meter_debug_exec(sl_zigbee_af_print_big_endian_eui64(deviceId));
  sl_zigbee_af_gbcs_gas_meter_println("");
...
static void setSleepyMeterState(uint8_t newState)
{
  ...
  switch (state) {
    case INITIAL_STATE:
    case REGISTRATION_COMPLETE:
      sl_zigbee_af_event_set_delay_qs(&sleepyMeterEvent, (INITIAL_STATE_EVENT_INTERVAL_SECONDS << 2));
      break;
    case MIRROR_READY:
      sl_zigbee_af_event_set_delay_qs(&sleepyMeterEvent, (MIRROR_UPDATE_INTERVAL_SECONDS << 2));
      break;
    case SERVICE_DISCOVERY_STARTED:
    case MIRROR_CONFIGURATION_STARTED:
    case MIRROR_UPDATE_IN_PROGRESS:
    default:
      sl_zigbee_af_event_set_delay_qs(&sleepyMeterEvent, MIRROR_UPDATE_IN_PROGRESS_INTERVAL_QUARTER_SECONDS);
      break;
  }
}
...
static bool tunnelDestroy(void)
{
  sl_zigbee_af_gbcs_gas_meter_println("GSME: TunnelDestroy");

  sl_zigbee_af_status_t status = SL_ZIGBEE_ZCL_STATUS_NOT_FOUND;
  if (tunnel.state != UNUSED_TUNNEL) {
    status = sl_zigbee_af_tunneling_client_close_tunnel(tunnel.tunnelId);
    if (status == SL_ZIGBEE_ZCL_STATUS_SUCCESS || status == SL_ZIGBEE_ZCL_STATUS_NOT_FOUND) {
      tunnel.state = UNUSED_TUNNEL;
                           ...
                           uint8_t *data)
{
  ...
  sl_zigbee_af_gbcs_gas_meter_print("GSME: TunnelSendData [");
  sl_zigbee_af_gbcs_gas_meter_print_buffer(data, dataLen, false);
  sl_zigbee_af_gbcs_gas_meter_println("]");

  if (tunnel.state == ACTIVE_TUNNEL) {
    status = sl_zigbee_af_tunneling_client_transfer_data(tunnel.tunnelId, data, dataLen);
...
static bool requestTunnel(void)
{
  ...
  status = sl_zigbee_af_tunneling_client_request_tunnel(tunnel.remoteNodeId,
                                                        GSME_ENDPOINT,
...
static void handleRemotePartyMessage(uint16_t gbzCommandLen, uint8_t *gbzCommand)
{
  sl_zigbee_af_gbz_message_parser_state_t gbzParser = { 0 };
  bool validGbzMessage;
  uint16_t headerLen = 0;
  uint16_t gbzResponseLen = 0;
  sl_zigbee_af_gbz_message_creator_state_t gbzCreator = { 0 };
  ...
  sl_zigbee_af_copy_int8u(message, headerLen, TUNNEL_MANAGER_HEADER_PUT);
  headerLen += 1;

  // Add any non GBZ header data from the request
#if GBZ_OFFSET != 0
  memcpy(&(message[headerLen]), gbzCommand, GBZ_OFFSET);
  headerLen += GBZ_OFFSET;
  gbzCommand += GBZ_OFFSET;
  gbzCommandLen -= GBZ_OFFSET;
#endif

  validGbzMessage = sl_zigbee_af_gbz_message_controller_parser_init(&gbzParser,
  ...
  if (validGbzMessage) {
    ...
    sl_zigbee_af_gbz_message_controller_creator_init(&gbzCreator,
    ...
    while (sl_zigbee_af_gbz_message_controller_has_next_command(&gbzParser)) {
      sl_zigbee_af_gbz_zcl_command_t zclRequest = { 0 };
      sl_zigbee_af_gbz_zcl_command_t zclResponse = { 0 };

      uint8_t zclResponsePayload[2];
      sl_zigbee_af_gbz_message_controller_next_command(&gbzParser, &zclRequest);

      sl_zigbee_af_gbcs_gas_meter_println("GSME: received the following ZCL request");
      sl_zigbee_af_gbz_message_controller_print_command_info(&zclRequest);
      ...
      zclResponsePayload[1] = SL_ZIGBEE_ZCL_STATUS_SUCCESS;
      ...
      sl_zigbee_af_gbcs_gas_meter_println("GSME: responding with the following ZCL response");
      sl_zigbee_af_gbz_message_controller_print_command_info(&zclResponse);

      sl_zigbee_af_gbz_message_controller_append_command(&gbzCreator, &zclResponse);
    }

    result = sl_zigbee_af_gbz_message_controller_creator_assemble(&gbzCreator);
    gbzResponseLen  = result->payloadLength;
    sl_zigbee_af_gbz_message_controller_creator_cleanup(&gbzCreator);
    sl_zigbee_af_gbz_message_controller_parser_cleanup(&gbzParser);
  } else {
    // This is not a valid GBZ message so for testing purposes we'll just echo
    // the received data back.
    if (gbzCommandLen > 0) {
      memcpy(&(message[headerLen]), gbzCommand, gbzCommandLen);
...
// sl_zigbee_af_find_devices_by_profile_and_cluster below. This callback is not part of
// the application framework callbacks, it is used by the service discovery
// libraries API.
static void serviceDiscoveryCallback(const sl_zigbee_af_service_discovery_result_t* result)
{
  if (state == SERVICE_DISCOVERY_STARTED
      && sl_zigbee_af_have_discovery_response_status(result->status)) {
    uint8_t i;
    uint8_t physAttId[] = {
    ...
    for (i = 0; i < epList->count; i++) {
      sl_zigbee_af_fill_command_global_client_to_server_read_attributes(ZCL_BASIC_CLUSTER_ID,
                                                                        physAttId,
                                                                        sizeof(physAttId));
      sl_zigbee_af_set_command_endpoints(GSME_ENDPOINT, epList->list[i]);
      sl_zigbee_af_send_command_unicast(SL_ZIGBEE_OUTGOING_DIRECT, result->matchAddress);
    }
  } else if (result->status == SL_ZIGBEE_AF_BROADCAST_SERVICE_DISCOVERY_COMPLETE) {
    sl_zigbee_af_app_println("service discovery complete.");
  }
}

bool sl_zigbee_af_simple_metering_cluster_request_mirror_response_cb(sl_zigbee_af_cluster_command_t *cmd)
{
  ...
  if (endpointId == 0xffff) {
    sl_zigbee_af_app_println("Mirror add FAILED");
  } else {
    if (state != MIRROR_READY) {
      ...
      sl_zigbee_af_fill_command_simple_metering_cluster_configure_mirror(issuerEventId,
                                                                         ...
                                                                         true,
                                                                         SL_ZIGBEE_ZCL_NOTIFICATION_SCHEME_PREDEFINED_NOTIFICATION_SCHEME_B);
      sl_zigbee_af_set_command_endpoints(GSME_ENDPOINT, mirrorEndpoint);
      sl_zigbee_af_send_command_unicast(SL_ZIGBEE_OUTGOING_DIRECT, mirrorAddress);
...
bool sl_zigbee_af_simple_metering_cluster_mirror_removed_cb(sl_zigbee_af_cluster_command_t *cmd)
{
                                                                                   ...
                                                                                   sl_service_function_context_t *context)
{
  ...
  if (!cmd->mfgSpecific) {
    switch (cmd->commandId) {
      ...
      {
        wasHandled = sl_zigbee_af_simple_metering_cluster_request_mirror_response_cb(cmd);
        break;
      }
      case ZCL_MIRROR_REMOVED_COMMAND_ID:
      {
        wasHandled = sl_zigbee_af_simple_metering_cluster_mirror_removed_cb(cmd);
        break;
      }
    }
  }
