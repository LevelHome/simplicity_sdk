 ...
 */
static inline mesh_addr_type_t mesh_addr_type(mesh_addr_t addr)
{
  switch ((addr >> 14) & 3) {
    case 0:
...
#define MESH_MESSAGE_FLAG_NONE 0x00
 ...
 */
typedef sl_status_t (*provisioner_event_handler_cb)(mesh_prov_event_type_t event_type,
                                                    const uint8_t *uuid,
                                                    uint8_t len,
                                                    const uint8_t *data);
 ...
 */
typedef struct {
  uint8_t page_number; /**< Number of the Composition Data page */
  size_t size;          /**< Length of the page data */
 ...
 */
typedef struct {
  uint8_t page_number; /**< Number of the Model Metadata page */
  size_t size;         /**< Length of the page data */
  const uint8_t *data; /**< Page content */
