...
static void sleeptimer_cb(__attribute__((unused)) sl_sleeptimer_timer_handle_t *handle, __attribute__((unused)) void *data);
...
static sl_sleeptimer_timer_handle_t my_sleeptimer_handle = { 0 };
...
static void sleeptimer_cb(__attribute__((unused)) sl_sleeptimer_timer_handle_t *handle, __attribute__((unused)) void *data)
{
  // Triggered when button is held down for longer than "short press".
  app_event_enqueue(EVENT_APP_KEY01_HOLD);
}
 ...
 * - `EVENT_APP_KEY01_SHORT_PRESS` for short press.
 * - `EVENT_APP_KEY01_HOLD` if button is pressed down longer than short press. (Triggered by a timer.)
 * - `EVENT_APP_KEY01_RELEASE` if button was released after an EVENT_APP_KEY01_HOLD event.
 */
void app_button_press_btn_0_handler(uint8_t duration)
{
  ...
  switch (duration) {
    ...
    case APP_BUTTON_PRESS_DURATION_SHORT:
      ...
      // Stop timer to avoid Key Down event.
      sl_sleeptimer_stop_timer(&my_sleeptimer_handle);
      break;
    default: // Any press duration longer than short press.
