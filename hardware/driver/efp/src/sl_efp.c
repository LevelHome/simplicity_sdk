...
static void enable_i2c_pins(sl_efp_handle_t handle, bool enable)
{
  ...
  if (enable) {
    ...
    sl_clock_manager_disable_bus_clock(handle->i2c_clock);
  }
}
...
sl_status_t sl_efp_deinit(sl_efp_handle_t handle)
{
  if (host_efp == handle) {
    ...
    EMU_EFPIntDisable(1U << _EMU_EFPIEN_EFPIEN_SHIFT);
#endif
...
sl_status_t sl_efp_emu_ldo_enable(sl_efp_handle_t handle, bool enable)
{
  if (handle->init_data.is_host_efp) {
    ...
    if (enable) {
      sli_se_mailbox_command_add_parameter(&command, 0);           // Value
    } else {
      sli_se_mailbox_command_add_parameter(&command, 1 << 14);     // Value
    }

    sli_se_mailbox_execute_command(&command);
    if (sli_se_mailbox_read_response() != SLI_SE_RESPONSE_OK) {
      return SL_STATUS_FAIL;
    }
    return SL_STATUS_OK;
...
sl_status_t sl_efp_init(sl_efp_handle_t handle, const sl_efp_init_data_t *init)
{
  ...
  if ( false ) {
    ...
    handle->i2c_clock = SL_BUS_CLOCK_I2C0;
    ...
    handle->i2c_clock = SL_BUS_CLOCK_I2C1;
#if defined(_SILICON_LABS_32B_SERIES_2)
    ...
    handle->i2c_clock = SL_BUS_CLOCK_I2C2;
#if defined(_SILICON_LABS_32B_SERIES_2)
